...
function buildHosts(hosts, analysis, title = 'สถานะ Host', icon = '💻', analyzeText = null) {
  ...
  const online  = hosts.filter((h) => h.available === 1 || h.online).length;
  const offline = hosts.filter((h) => h.available === 2 || h.online === false).length;
  const hc      = offline ? COLOR.red : COLOR.green;

  const rows = hosts.slice(0, 10).map((h) => {
    const isOn = h.available === 1 || h.online === true;
    const statusIcon  = isOn ? '🟢' : (h.available === 2 || h.online === false ? '🔴' : '❓');
    const statusLabel = h.status || (isOn ? 'ออนไลน์' : 'ออฟไลน์');
    ...
    return hbox([
      ...
      txt(statusLabel.replace(/[🟢🔴❓]/g, '').trim(), 'xs', isOn ? COLOR.green : COLOR.red,
