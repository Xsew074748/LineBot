...
// Sanitize text สำหรับ Flex message — ลบ newline/carriage return ที่ทำให้ 400 error
function sanitizeFlex(text, maxLen = 200) {
  return String(text || '')
    .replace(/[\r\n]+/g, ' ')    // ลบ newline/carriage return แทนด้วย space
    .replace(/\s+/g, ' ')        // ลบ multiple spaces
    .trim()
    .slice(0, maxLen);
}
...
// ── ปุ่ม 🤖 ประจำแถว device — postback data = "analyze:{type}:{name}:{ip}" ─────
// ใช้ text+action แทน box เพื่อประหยัด byte (การ์ด 8 แถวต้องไม่เกิน FLEX_SAFE_BYTES)
// LINE จำกัด postback data 300 chars — slice กันไว้
function aiRowBtn(deviceType, name, ip) {
  const data = `analyze:${deviceType}:${name}:${ip || '-'}`.slice(0, 300);
  return {
    type: 'text', text: '🤖', size: 'lg', flex: 0, margin: 'sm', gravity: 'center',
    action: { type: 'postback', label: '🤖', data, displayText: `🤖 วิเคราะห์ ${name}`.slice(0, 300) },
  };
}

// ── Footer รายการ: แถว ← Back / Next → ในการ์ด + ปุ่มวิเคราะห์ทั้งหมด ─────────
// pg = { page, totalPages, hasPrev, hasNext } — pageCmd เช่น "host" → กดส่ง "host 2"
function navBtn(label, text2) {
  return {
    type: 'button', style: 'secondary', height: 'sm', flex: 1,
    action: { type: 'message', label, text: text2 },
  };
}

function listFooter(pageCmd, pg, analyzeText, analysis = null) {
  if (analysis) return aiFooter(analysis);

  const contents = [];
  if (pageCmd && pg && pg.totalPages > 1) {
    // บรรทัด 1: เลขหน้า (center)
    contents.push(txt(`${pg.page}/${pg.totalPages}`, 'sm', '#888888', { align: 'center' }));

    // บรรทัด 2: Back ชิดซ้าย / Next ชิดขวา — ด้านที่ไม่มีปุ่มใช้ filler ดันตำแหน่ง
    const filler = { type: 'filler', flex: 1 };
    const navRow = [
      pg.hasPrev ? navBtn('← ก่อนหน้า', `${pageCmd} ${pg.page - 1}`) : filler,
      pg.hasNext ? navBtn('ถัดไป →',   `${pageCmd} ${pg.page + 1}`) : filler,
    ];
    contents.push(hbox(navRow, { spacing: 'sm', margin: 'sm' }));
  }

  // บรรทัด 3: ปุ่มวิเคราะห์ทั้งหมด
  if (analyzeText) {
    contents.push({
      type: 'button',
      action: { type: 'message', label: '🤖 วิเคราะห์ทั้งหมด', text: analyzeText },
      style: 'primary', color: COLOR.blue, height: 'sm',
      ...(contents.length && { margin: 'sm' }),
    });
  }
  if (!contents.length) return null;
  return vbox('#F8F9FA', contents, { paddingAll: '10px' });
}
...
// processedData = { counts, clusters, topItems, moreCount, hiddenCount, pageInfo? }
// pageInfo = { page, totalPages } — แสดง "(หน้า X/Y)" ต่อท้าย title เมื่อแบ่งหน้า
// opts.pageCmd + opts.pg: ปุ่มเปลี่ยนหน้าในการ์ด (แทน Quick Reply)
function buildAlerts(processedData, analysis, analyzeText = null, opts = {}) {
  const { counts, clusters, topItems, moreCount, hiddenCount, pageInfo } = processedData;
  ...
  const items = topItems.map((p) => {
    ...
    return hbox([
      { type: 'box', layout: 'vertical', flex: 1, contents: [
        ...
        txt(sanitizeFlex(p.description, 150), 'sm', '#222222', { weight: 'bold', wrap: true, margin: 'xs' }),
        txt(`📍 ${sanitizeFlex(p.host, 80)}`, 'xs', '#666666', { margin: 'xs' }),
      ]},
      aiRowBtn('alert', p.host, p.ip),
    ], { margin: 'sm', paddingAll: '10px', backgroundColor: c + '15', cornerRadius: '8px' });
  });
  ...
  const pageSuffix = pageInfo && pageInfo.totalPages > 1 ? ` (หน้า ${pageInfo.page}/${pageInfo.totalPages})` : '';
  const headerContents = [
    txt(total ? `⚠️ การแจ้งเตือนระบบ${pageSuffix}` : '✅ ระบบปกติ', 'lg', COLOR.white, { weight: 'bold' }),
    hbox([
...
// pageInfo = { page, totalPages, total } — items คือหน้าที่แบ่งแล้วจาก index.js
function buildAlertWarning(items, pageInfo = null) {
  ...
  const rows  = shown.map((p) => {
    ...
    return vbox(c + '10', [
      ...
      txt(sanitizeFlex(p.description, 120), 'xs', '#222222', { weight: 'bold', wrap: true, margin: 'xs' }),
      txt(`📍 ${sanitizeFlex(p.host, 70)}`, 'xxs', '#666666', { margin: 'xs' }),
    ], { margin: 'sm', paddingAll: '8px', cornerRadius: '6px' });
  });
  ...
  return bubble(
    vbox(COLOR.orange, [
      txt(`🟠 Alert: Average / Warning${pageSuffix}`, 'lg', COLOR.white, { weight: 'bold' }),
      hbox([
        txt(`พบ ${totalCount} รายการ`, 'sm', COLOR.white, { flex: 1 }),
        txt(nowTH(), 'xs', '#999999', { align: 'end', flex: 0 }),
      ], { margin: 'sm' }),
    ]),
...
// ── Host / Camera Message ──────────────────────────────────────────────────────
// opts.statsFrom: array เต็มสำหรับนับ total/online/offline ใน header
// เมื่อ hosts เป็นแค่หน้าเดียวจาก pagination (ไม่ส่ง = นับจาก hosts ตามเดิม)
// opts.deviceType: 'host' | 'ap' — ใส่ปุ่ม 🤖 ข้างแต่ละแถว (postback analyze:...)
// opts.pageCmd + opts.pg: แสดงปุ่ม ← Back / Next → ในการ์ด (แทน Quick Reply)
function buildHosts(hosts, analysis, title = 'สถานะ Host', icon = '💻', analyzeText = null, opts = {}) {
  const isOnline = (h) =>
    h.available === 1 || h.online === true || h.status === 'up' || h.status === '🟢 เชื่อมต่อ' || h.status === '🟢 ออนไลน์';

  const statsSrc = opts.statsFrom || hosts;
  const total   = statsSrc.length;
  const online  = statsSrc.filter(isOnline).length;
  const offline = total - online;
  const hc      = offline ? COLOR.red : COLOR.green;

  // Layout 2 บรรทัด/แถว: บน = 🟢 ชื่อ + 🤖 | ล่าง = (IP)
  const rows = hosts.slice(0, 10).map((h) => {
    const on = isOnline(h);
    // Omada AP มี h.ip ตรง ๆ — Zabbix host มี IP อยู่ใน interfaces[]
    const ip = h.ip || h.interfaces?.[0]?.ip || null;

    const topRow = hbox([
      txt(on ? '🟢' : '🔴', 'sm', COLOR.white, { flex: 0, gravity: 'center' }),
      txt(h.name, 'sm', '#222222', { flex: 1, weight: 'bold', wrap: true, margin: 'sm' }),
      aiRowBtn(opts.deviceType || 'host', h.name, ip),
    ]);

    const contents = [topRow];
    if (ip) contents.push(txt(`(${ip})`, 'xs', '#888888', { margin: 'xs' }));

    return vbox('#FAFAFA', contents, { margin: 'sm', paddingAll: '8px', cornerRadius: '6px' });
  });

  // นับจาก hosts (รายการที่แสดงจริง) ไม่ใช่ total — เมื่อ paginate ปุ่ม QR ทำหน้าที่แทน
  if (hosts.length > 10) rows.push(txt(`… และอีก ${hosts.length - 10} รายการ`, 'xs', '#888888', { align: 'center', margin: 'sm' }));
...
// ── Camera Sites Overview (ระดับ 1 ของคำสั่ง "กล้อง") ─────────────────────────
// siteStats = [{ site, total, online, offline }]
// สีตามสถานะไซต์: เขียว=ออนไลน์ทั้งหมด, แดง=ออฟไลน์ทั้งหมด, เหลือง=ปนกัน
function siteStatusColor(s) {
  if (s.offline === 0) return COLOR.green;
  if (s.online === 0)  return COLOR.red;
  return COLOR.yellow;
}
...
function buildCameraSites(siteStats, analyzeText = null) {
  const total   = siteStats.reduce((n, s) => n + s.total, 0);
  const online  = siteStats.reduce((n, s) => n + s.online, 0);
  const offline = total - online;
  const hc      = offline === 0 ? COLOR.green : online === 0 ? COLOR.red : COLOR.yellow;

  const rows = siteStats.map((s) => {
    const c    = siteStatusColor(s);
    const icon = s.offline === 0 ? '🟢' : s.online === 0 ? '🔴' : '🟡';
    return {
      type: 'box', layout: 'horizontal', margin: 'sm', paddingAll: '10px',
      backgroundColor: SITE_TINT[c], cornerRadius: '8px',
      // กดแถว → ดูกล้องในไซต์ (เหมือนกดปุ่ม Quick Reply)
      action: { type: 'message', label: s.site, text: `กล้อง:${s.site}` },
      contents: [
        txt(icon, 'md', COLOR.white, { flex: 0, gravity: 'center' }),
        { type: 'box', layout: 'vertical', flex: 1, margin: 'sm', contents: [
          txt(s.site, 'sm', c === COLOR.yellow ? '#8A6D00' : c, { weight: 'bold' }),
          txt(`กล้อง ${s.total} ตัว`, 'xxs', '#888888'),
        ]},
        { type: 'box', layout: 'vertical', flex: 0, contents: [
          txt(`🟢 ${s.online}`,  'xs', s.online  ? COLOR.green : '#BBBBBB', { align: 'end', weight: 'bold' }),
          txt(`🔴 ${s.offline}`, 'xs', s.offline ? COLOR.red   : '#BBBBBB', { align: 'end' }),
        ]},
        txt('›', 'xl', '#BBBBBB', { gravity: 'center', flex: 0, margin: 'sm' }),
      ],
    };
  });

  if (!rows.length) rows.push(txt('ℹ️ ไม่พบข้อมูลกล้อง', 'sm', '#888888', { align: 'center' }));
  rows.push(txt('กดไซต์หรือปุ่มด้านล่างเพื่อดูกล้องรายตัว', 'xs', '#AAAAAA', { align: 'center', margin: 'md' }));

  return bubble(
    vbox(hc, [
      txt('📷 สถานะกล้องรายไซต์', 'lg', COLOR.white, { weight: 'bold' }),
      hbox([
        txt(`ทั้งหมด ${total} ตัว · ${siteStats.length} ไซต์`, 'xs', COLOR.white, { flex: 1 }),
        txt(nowTH(), 'xs', '#FFFFFFAA', { align: 'end', flex: 0 }),
      ], { margin: 'sm' }),
      hbox([
        txt(`🟢 ${online} ออนไลน์`, 'xs', COLOR.white, { flex: 1 }),
        txt(`🔴 ${offline} ออฟไลน์`, 'xs', COLOR.white, { flex: 1 }),
      ], { margin: 'xs' }),
    ]),
    vbox(COLOR.white, rows, { spacing: 'xs', paddingAll: '12px' }),
    listFooter(null, null, analyzeText)
  );
}

// ── Camera Site Detail (ระดับ 2 — กล้องในไซต์ แบ่งหน้าจาก index.js) ──────────
// cams = รายการเฉพาะหน้านี้, pg = { page, totalPages }, allSiteCams = ทั้งไซต์ (นับ header)
function buildSiteCameras(site, cams, pg, allSiteCams) {
  const isOn    = (c) => (c.available !== undefined ? c.available === 1 : c.online === true);
  const total   = allSiteCams.length;
  const online  = allSiteCams.filter(isOn).length;
  const offline = total - online;
  const hc      = offline === 0 ? COLOR.green : online === 0 ? COLOR.red : COLOR.yellow;

  // Layout 2 บรรทัด/แถว (เหมือน buildHosts): บน = 🟢 ชื่อ + สถานะ + 🤖 | ล่าง = (IP)
  const rows = cams.map((c) => {
    const on = isOn(c);
    const ip = c.ip || c.interfaces?.[0]?.ip || null;
    // ตัด prefix ไซต์ออกจากชื่อ เช่น "นเรศวร.ทางเข้า1" → "ทางเข้า1"
    const shortName = String(c.name || '').startsWith(site)
      ? String(c.name).slice(site.length).replace(/^[.\-_\s]+/, '')
      : c.name;

    const topRow = hbox([
      txt(on ? '🟢' : '🔴', 'sm', COLOR.white, { flex: 0, gravity: 'center' }),
      txt(shortName || c.name, 'sm', '#222222', { weight: 'bold', flex: 1, margin: 'sm', wrap: true }),
      txt(on ? 'ออนไลน์' : 'ออฟไลน์', 'xs', on ? COLOR.green : COLOR.red,
        { weight: 'bold', align: 'end', gravity: 'center', flex: 0 }),
      aiRowBtn('camera', c.name, ip),
    ]);

    const contents = [topRow];
    if (ip) contents.push(txt(`(${ip})`, 'xs', '#888888', { margin: 'xs' }));

    return vbox(on ? '#F7FDF9' : '#FFF5F5', contents,
      { margin: 'sm', paddingAll: '8px', cornerRadius: '6px' });
  });

  if (!rows.length) rows.push(txt('ℹ️ ไม่พบข้อมูลกล้อง', 'sm', '#888888', { align: 'center' }));

  const pageSuffix = pg.totalPages > 1 ? ` (หน้า ${pg.page}/${pg.totalPages})` : '';
  return bubble(
    vbox(hc, [
      txt(`📷 กล้อง ${site}${pageSuffix}`, 'lg', COLOR.white, { weight: 'bold' }),
      hbox([
        txt(`ทั้งหมด ${total} ตัว`, 'xs', COLOR.white, { flex: 1 }),
        txt(nowTH(), 'xs', '#FFFFFFAA', { align: 'end', flex: 0 }),
      ], { margin: 'sm' }),
    ...
    vbox(COLOR.white, rows, { spacing: 'xs', paddingAll: '12px' }),
    listFooter(`กล้อง:${site}`, pg, 'วิเคราะห์กล้อง')
  );
}
...
function buildOfflineCameras(data, cacheAgeMin = null, analyzeText = null) {
  ...
  if (total === 0) {
    ...
    // ≤ 30 ตัว: detail list — ปุ่ม 🤖 ขวาของแต่ละแถว
    rows = cameras.map((c) =>
      hbox([
        { type: 'box', layout: 'vertical', flex: 1, contents: [
          hbox([
        ...
        aiRowBtn('camera', c.name, c.ip),
      ], { margin: 'sm', cornerRadius: '8px', paddingAll: '10px', backgroundColor: '#FFF5F5' })
    );
  }
  ...
  // pg สำหรับปุ่ม ← Back / Next → ในการ์ด (เฉพาะ detail list ที่แบ่งหน้า)
  const pg = totalPages > 1
  ...
  return bubble(
    vbox(hc, [
      ...
      txt(subText, 'xs', '#999999', { margin: 'sm', wrap: true }),
    ]),
    vbox(COLOR.white, rows, { spacing: 'xs', paddingAll: '12px' }),
    listFooter('กล้องดับ', pg, analyzeText)
  );
}
...
function buildCorrelation(groups, analyzeText = null) {
  ...
  return bubble(
    vbox(hc, [
      txt('🔗 Cross-System Correlation', 'lg', COLOR.white, { weight: 'bold' }),
      txt(title, 'xs', '#999999', { margin: 'sm', wrap: true }),
      txt(nowTH(), 'xs', '#AAAAAA', { margin: 'xs' }),
    ]),
...
// extras ขึ้นก่อน standard เพื่อให้ปุ่ม context (เช่น เปลี่ยนหน้า) เห็นทันทีไม่ต้องเลื่อน
